...
static int init_entry_linux(struct flb_in_netinfo_config *ctx)
{
    int i;
...
static inline int is_specific_interface(struct flb_in_netinfo_config *ctx,
                                        char* interface)
{
...
static int parse_proc_line(char *line,
                           struct flb_in_netinfo_config *ctx)
{
    ...
    split = flb_utils_split(line, ' ', NET_FILE_LINE_LEN);
...
static int in_netinfo_collect_linux(struct flb_input_instance *i_ins,
                                struct flb_config *config, void *in_context)
{
    struct flb_in_netinfo_config *ctx = in_context;
    FILE *fp = NULL;
    char proc_file[100];
    char line[NET_FILE_LINE_LEN] = {0};
    char key_name[NET_FILE_LINE_LEN] = {0};
    int  key_len;
    int i;
    ...
    snprintf(proc_file, sizeof(proc_file), "%s/%s", ctx->proc_path, "net/dev");
    fp = fopen(proc_file, "r");
    if (fp == NULL) {
        flb_errno();
        flb_plg_error(ctx->ins, "cannot open %s", proc_file);
        return -1;
    }
    while(fgets(line, NET_FILE_LINE_LEN-1, fp) != NULL){
        parse_proc_line(line, ctx);
    }

    if (ctx->first_snapshot == FLB_TRUE) {
        /* if in_netinfo are called for the first time, assign prev with now */
...
static int configure(struct flb_in_netinfo_config *ctx,
...
static int config_destroy(struct flb_in_netinfo_config *ctx)
{
    flb_free(ctx->entry);
    flb_free(ctx);
    return 0;
...
static int in_netinfo_collect(struct flb_input_instance *i_ins,
                            struct flb_config *config, void *in_context)
{
    return in_netinfo_collect_linux(i_ins, config, in_context);
}

static int in_netinfo_init(struct flb_input_instance *in,
                         struct flb_config *config, void *data)
{
    ...
    struct flb_in_netinfo_config *ctx = NULL;
    (void) data;

    /* Allocate space for the configuration */
    ctx = flb_calloc(1, sizeof(struct flb_in_netinfo_config));
    if (!ctx) {
        flb_errno();
        return -1;
    }
...
static int in_netinfo_exit(void *data, struct flb_config *config)
{
    (void) *config;
    struct flb_in_netinfo_config *ctx = data;
...
/* Plugin reference */
struct flb_input_plugin in_netinfo_plugin = {
    .name         = "netinfo",
    .description  = "Network Interface Usage Info",
    .cb_init      = in_netinfo_init,
    .cb_pre_run   = NULL,
    .cb_collect   = in_netinfo_collect,
    .cb_flush_buf = NULL,
    .cb_exit      = in_netinfo_exit,
    .flags        = 0,
};
